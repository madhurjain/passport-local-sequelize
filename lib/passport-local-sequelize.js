...
var attachToUser = function (UserSchema, options) {
    ...
    var attachToSingleUser = function(user) {
        ...
        user.setActivationKey = function (cb) {
            var self = this;
        ...
        user.authenticate = function (password, cb) {
            var self = this;
    ...
    var attachFunc = function(user, op, next){
        if (user.forEach){
            user.forEach(function(usr){ attachToSingleUser(usr); });
        } else {
            attachToSingleUser(user);
        }
    ...
    UserSchema.afterRestore(attachFunc);

    UserSchema.afterFind(attachFunc);

    UserSchema.afterCreate(attachFunc);
};
